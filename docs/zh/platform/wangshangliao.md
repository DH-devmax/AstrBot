...
AstrBot 管理员可在私聊中要求模型查询群、搜索成员，再执行禁言、解禁、踢出、公告和全员禁言／解除。模型需要支持工具调用，并启用本插件。普通用户、群聊和其他平台不提供此管理工具。

流程：私聊 → AstrBot 管理员检查 → 插件固定工具 → 群／成员目录快照 → 共用命令执行服务 → 接入器最终授权与平台权限检查 → 持久化操作结果 → 纯文本脱敏回复。选群和成员编号十分钟有效；重名须询问用户，不按昵称猜身份。授权仍在 Dashboard 配置，聊天不能修改授权。已接受不是已确认；未知结果只查询、不重试。撤回仅保留自动违规入口。
...
![追踪窄屏验收](/images/wangshangliao/trace-acceptance-mobile.png)
...
## 当前能力与部署检查（2026-09-18）

- 登录支持密码登录与短信登录。短信验证码由用户提供；自动滑块失败时保留人工验证入口。认证成功、保存成功和消息在线是三个不同状态。
- 多群选择已解除后端单群限制；每群单独控制回复和群管动作。修改后保存，再检查连接状态；在线不代表所有动作有权限。
- 私聊回复可单独关闭，群内回复按群设置，开启后仍要求明确提及。已配置的旺商聊机器人相互排除，包括停用实例，防止循环对话。
- 成员目录使用统一分页，检查重复游标、身份冲突和缺失分页信息；请求失败不使用部分身份映射执行群管。云端私聊目录仍待确认，本地已知会话不是全部云端对话。
- 主动发送开关默认关闭。目前仅完成文本入口与 outbox 复用，尚无独立的按目标授权配置；定时任务稳定操作 ID、结果查询及私聊会话格式仍需完善。当前按会话与正文生成去重键，同会话重复相同正文可能返回旧结果，不应当视为完整定时推送能力。
- `accepted` 表示服务端接受，不表示对端确认；`unknown` 不自动重发。媒体、部分群管状态回读、24 小时连续运行及续期验收仍未完成。

### Linux 迁移检查

使用下文源码或 Docker 命令安装。不要复制 Windows EXE、DLL、`.pyd` 或 macOS 虚拟环境；在目标系统重新安装对应架构的依赖。Linux 路径区分大小写，模型、Node 脚本及配置必须完整部署。
...
1. 使用 Python 3.12+，运行 `uv sync`。在 Dashboard 目录运行 `pnpm install`、`pnpm generate:api` 和 `pnpm build`。
2. 管理员设置 `ASTRBOT_WANGSHANGLIAO_CONFIG`，指向服务器上的协议配置 JSON 文件。文件权限在 macOS/Linux 上须为 `0600`；Windows 使用仅服务账号可读的 ACL。
3. 配置字段为 `origin`（HTTPS 根地址）、`headers`、`metadata`（14 个整数）、`signing_seed`、`body_key`、`message_key`、`server_key`（均为标准 Base64 编码的 32 字节协议密钥）、`captcha_id`、`app_key`、`device_id`。这些值必须由部署管理员提供，不能从算法名称推算。配置缺失时登录返回 `deployment_missing`，配置不合格返回 `deployment_invalid`。
4. 启动 AstrBot。构建后的前端需要按当前部署方式使用 `dashboard/dist`；不要只更新 Python 后端而继续使用旧页面。

协议配置不进入机器人普通配置，也不通过浏览器接口返回。不要复制旧工作台数据库、密封会话文件或抓包数据。密码仅用于当前登录请求，默认不保存。
...
![开发门禁窄屏保存回显](/images/wangshangliao/developer-window-mobile.png)
...
![旺商聊与 AstrBot 接入架构](/images/wangshangliao/architecture-layers.svg)

管理员命令已实现，统一使用 `/群管` 前缀；真实验收与代码实现分开记录。

| 归属 | 负责 | 不负责 |
| --- | --- | --- |
| AstrBot | `/sid`、管理员列表、命令权限、模型、人格、知识库、会话路由、配置和日志 | 旺商聊协议实现 |
...
1. 打开机器人页面，点击创建机器人，选择 **旺商聊**。
2. 点击 **开始登录**，输入账号和密码。默认使用内置本地求解器处理滑块验证；若求解器不可用或平台要求人工交互，页面会保留人工验证回退。测试夹具不会绕过真实平台的人机验证。
3. 平台要求设备验证时，输入短信验证码；验证码仍按当前配置使用内置求解器，必要时可人工完成。重发按钮显示冷却时间。
4. 页面显示“账号认证成功”后，从自动读取的群聊列表勾选启用群，选择 AstrBot 配置文件并保存。空群列表表示不处理任何群。
5. 保存后才连接 NIM；账号认证成功并不等于消息服务已经在线。认证后页面自动滚动到固定黄色未保存提示，点击“保存登录并连接”。刷新或离开页面前会提醒登录尚未保存；保存成功后提醒清除。实例详情分别展示在线、重连中、需要登录、已停止、连接错误。

![原生登录创建表单](/images/wangshangliao/login-zh.png)

登录事务有效期为十分钟，绑定 Dashboard 用户和预分配的实例 ID。浏览器获得一次性会话引用；保存时才将会话绑定到实例。重复保存同一个已成功提交的引用不会创建第二个实例。

## 使用与状态

- 只接受已启用群中、通过业务身份和 NIM 身份映射验证的消息；过滤自己的消息。
- 群回复仅认当前登录会话 NIM ID 的结构化提及，再转换成 AstrBot 业务账号的 At 组件。请在旺商聊客户端选择对应成员完成 @；手打 `@DH`、昵称或账号数字不作为身份依据。只 @ 其他成员不回复，同时明确 @ 多个机器人时各自按配置判断。
- 普通消息只保留到内置群上下文；是否在后续模型请求中使用上下文，遵循所选配置文件的群上下文设置。普通消息不执行命令、不触发主动回复。
- 同群等待当前事件全部处理完再处理下一条；会话包含实例、账号和群身份。
- 服务端接受状态 `accepted` 不是对端送达。发送超时记为 `unknown`，不自动补发；中断生成记为 `needs_review`，不自动重新调用模型。
- 私信文本无需 @ 即进入 AstrBot；遵循所选配置的模型、权限与会话设置。私信与群聊、不同账号及不同联系人分别隔离，空群列表仍可处理私信。
- 私信仅支持经过认证的旺商聊文本封装；群管通过 AstrBot 配置的动作授权与规则执行，媒体尚未接入；主动文本发送已有基础接口，完整定时发送语义仍待补齐。

## 停用、退出、回退

**停用**后保存配置：关闭连接和处理任务，保留加密恢复会话。**退出账号**：立即关闭连接并删除本地恢复会话；历史去重与收发账本保留。**重新登录**会先停止旧连接，验证成功并保存后替换会话。
...
- 保存账号不等于消息连接在线；先确认机器人启用及运行状态。
- 空启用群列表不会处理群消息；私信按独立会话处理。
- 已保存机器人可直接刷新群列表；停用时只读取业务目录，不启动消息连接。有效会话过期后仍需重新登录。
- 群管执行前校验权限；全群禁言独立状态回读仍待接入。文本发送不要求管理员，但仍受平台禁言限制。
- 实际群/私信收发、对端回执、重启恢复与旧系统回退需要真实联调。离线测试不代表这些项目已通过。
- 公告发布与回读已实现；媒体、撤回及历史知识迁移未实现；主动文本接口已实现，按目标授权和定时任务幂等仍待完善。
...
WSL 自动验证默认使用仓库内置求解器，不需要启动 7795 HTTP 服务。
ONNX 模型约 19 MB，随源码部署；首次验证才启动一个本地工作进程，复用模型和一个 Node.js 工作进程。
没有验证请求时不会主动求解。退出 AstrBot 时释放工作进程，超时或取消会终止当前工作进程。
...
2026-09-20 在“大海兼职群”通过已保存的临时授权，将一名普通成员从 `D1` 改为“名片测试”，再恢复 `D1`，两次均由完整成员目录回读确认。结束后恢复临时授权。手机端观察、自动进群和整批规范仍待验收。

Dashboard 鉴权接口的 `card_preview` 支持配对传入 `card_member` 与 `card_name` 生成单成员改名或原名恢复预览；仍排除受保护成员，执行绑定服务端预览。私聊 AI 不开放任意名片或成员列表参数。

在“消息平台 → 当前机器人 → 机器人群管授权”按群名选择群，勾选“修改普通成员群名片”并保存。回复开关、主动发送、授权、名片任务及测试群共用群名目录，显示群名与 ID，保存的仍是 ID；目录暂不可用时显示“群名待加载”并保留选择。

![群名授权回显](images/wangshangliao-group-labels.jpg)

页面顶部的目录完整性状态也显示群名与 ID；群名与状态分别来自群目录和运行实例，不将“目录完整”解释为所有动作均已授权。

![目录状态群名](images/wangshangliao-directory-labels.jpg)
“自动规范本群普通成员名片”默认关闭；开启后每五分钟检查完整成员目录，不调用 AI。
封禁、注销账号不参与名片修改，发送前再次检查。历史未知项若回读证实账号已封禁或注销，会保留未知结果并隔离该成员；新任务可处理其他成员，但不会重试隔离项，即使该账号后来恢复正常。
获得管理员身份不会自动授权其他群。自动处理排除机器人自己、所有已接入账号、群主与管理员。

手动入口为同页“群名片批量规范”：生成预览 → 核对原名和新名 → 执行此预览 → 刷新进度。
“停止待执行项”不能取消已发出的请求。预览十分钟有效，绑定当前账号和 Dashboard 用户。
名片在预览后变化会拒绝覆盖；未知或仅接受的结果停止后续修改，需先回读核对，不重复执行。
重启时仅核对不确定项，已确认项不重发。人工改过的已处理成员不会被自动反复覆盖，可重新手动预览。

优先取当前群名片，空白时取账号昵称；去除首尾空白，按 Unicode 字素取前两字符，保留组合字符与 Emoji。
不足两字符使用“大海群员＋六位随机数字”，检查完整目录中的重名并持久化；两字名允许重名。

私聊管理员通过原有 `wsl_private_management` 工具使用 `card_preview`、`card_execute`、`card_status`、`card_stop`。
人格的工具白名单须包含 `wsl_private_management`。先选择已验证群，明确请求批量预览，再用新消息要求执行该预览。
执行只接受服务端预览 ID，不接受模型提供的任意成员列表。群内仍只开放原有六项结构化 @ 管理命令。
查询、否定、条件、引用不能授权修改；存在歧义时应追问。程序门禁之外仍需真实模型语义验收。

```mermaid
flowchart TD
  A[Dashboard 逐群授权与开关] --> D[接入器最终授权与生命周期检查]
  B[管理员私聊 AI 意图] --> C[插件：目录选择与服务端预览]
  C --> E[用户新消息明确执行预览]
  E --> F[插件：持久化串行任务，每次间隔至少一秒]
  G[五分钟完整目录扫描，不调用 AI] --> F
  A --> G
  F --> D
  D --> H[固定 set-member-nickname 能力]
  H --> I[完整目录回读与结果台账]
  I --> F
  I --> J[接受／确认／拒绝／未知；未知停止]
```

当前验收边界：程序回归与浏览器验收应分别记录；已接入的普通测试身份仍属于受保护机器人，不能把管理员身份或开发消息门禁当作名片保护豁免。不对真实群全体成员直接做批量验收。

![390px card preview](images/wangshangliao-cards-narrow.jpg)

## 封禁／注销账号清理

原“群名片批量规范”入口现为“成员批量操作”。选择“封禁／注销清理”，逐群保存独立的清理动作授权后生成预览，核对并勾选确认，再执行。该操作仅移出群聊，不注销平台账号。默认关闭，不自动清理。

仅使用平台明确的封禁、注销状态；不按昵称判断。排除群主、管理员、机器人自身和已接入机器人。每次移出前重新核验状态、身份及授权，账号恢复正常时跳过。串行执行，可查看进度和停止；未知结果暂停并回读，不盲目补发，重启不续跑清理任务。
